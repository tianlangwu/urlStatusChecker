  ...
  readFile(
    ...
    return new Promise((resolve, reject) => {
      fs.readFile(fileString, async function (err, data) {
        if (err)
          return reject(
            "Can't find file name \"" + fileString + '" in the path',
          );
        ...
        if (filteredUrl != null) {
          if (filteredUrl.length != 0) {
            //if filter is not empty, compare each filter with each url and remove the matching urls
            for (let i = 0; i < filteredUrl.length; i++) {
              for (let j = 0; j < urls.length; j++) {
                if (
                  urls[j].startsWith(
                    filteredUrl[i].replace(/[^\x20-\x7E]/gim, ""),
                  )
                ) {
                  //replace used to remove control characters read from file
                  urls.splice(j, 1);
            ...
            return console.log(
              "Invalid ignore pattern, please check the ignore file content.",
            );
        }
